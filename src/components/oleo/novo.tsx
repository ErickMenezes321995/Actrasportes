...
interface NovoOleoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSalvar: (troca: TrocaOleo) => void;
  caminhoes: Caminhao[];
}

const formDataInicial: Omit<TrocaOleo, "id"> = {
  caminhaoId: "",
  dataTroca: "",
  quilometragem: 0,
  tipoOleo: "",
...
const NovoOleoModal: React.FC<NovoOleoModalProps> = ({
  isOpen,
  onClose,
  onSalvar,
  caminhoes,
  ...
  const [formData, setFormData] = useState<Omit<TrocaOleo, "id">>(formDataInicial);
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (isOpen) setFormData(formDataInicial);
  }, [isOpen]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erro ao criar troca de óleo: ${errorText}`);
      }

      const novaTroca: TrocaOleo = await response.json();

      toast({
        title: "Troca de óleo registrada com sucesso",
        status: "success",
        duration: 3000,
        isClosable: true,
      });
      ...
      console.error("Erro ao cadastrar troca de óleo:", error);
      toast({
        title: "Erro ao cadastrar troca de óleo",
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      ...
      <ModalContent borderRadius="4px" border="1px solid #a3bde3" boxShadow="0 2px 8px rgba(0,0,0,0.15)">
        ...
        >Nova Troca de Óleo</ModalHeader>
        <ModalCloseButton 
         color="#666666"
            ...
            <FormControl isRequired mb={2}>
              ...
              <Input
               name="tipoOleo"
                ...
                onChange={handleChange}
                placeholder="Ex: Sintético 5W-30"
                 fontSize="13px"
                border="1px solid #d0d0d0"
                borderRadius="3px"
                height="32px"
        ...
        >
          ...
          onClick={handleSubmit} isLoading={isSaving}>
            Salvar
          </Button>
        </ModalFooter>
      </ModalContent>
